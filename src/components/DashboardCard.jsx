...
function DashboardCard({card}) {
  return (
    <div 
      ...
      w-90vw rounded-sm bg-card_light pt-8 pb-6`}>
      <div className="flex items-center gap-2">
        <img src={card.icon} alt="facebook icon"/>
        <p className="font-color_light text-sm font-bold">@nathanf</p>
      </div>
      <div className='flex flex-col items-center'>
        <p className='text-5xl font-extrabold color-black'>{card.number}</p>
        <p className="font-color_light text-xs tracking-5 uppercase">{card.type}</p>
      </div>
      <div className="flex items-center gap-2">
        <img src={card.change > 0 ? upIcon : downIcon} alt="arrow pointing up"/>
